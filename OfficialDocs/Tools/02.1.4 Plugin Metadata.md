...
Plugin Metadata provides runtime context that controls dynamic behavior—specifically **dynamic descriptions** and **conditional registration**. It allows capabilities to adapt based on runtime state without changing code.

## What Is Plugin Metadata?

Plugin Metadata is a property bag passed to a plugin at registration time:
...
The plugin can then use these values for:
...
All metadata classes must implement `IPluginMetadata`:
...
- `description` in `[Collapse]` attribute
...
var agent = new AgentBuilder()
    .WithPlugin<SearchPlugin>(metadata)
    .Build();
```
...
```csharp
// Metadata definition
public class WebSearchMetadata : IPluginMetadata
{
...
// Plugin using metadata
[Collapse("Web search operations using {metadata.ProviderName}")]
...
// Registration
var agent = new AgentBuilder()
    .WithPlugin<WebSearchPlugin>(new WebSearchMetadata
    {
        ProviderName = "Google",
        MaxResults = 20,
        HasImageSearch = true,
...
4. **Keep metadata focused**: One metadata class per plugin; don't share across unrelated plugins.
...
public class Meta : IPluginMetadata
{
