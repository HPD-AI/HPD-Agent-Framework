...
public static SubAgent Create(
    ...
    params Type[] pluginTypes  // Optional plugins for the SubAgent
)
```
...
## SubAgents with Plugins

SubAgents can have their own plugins:
...
public SubAgent FileAnalyzer()
{
    ...
    return SubAgentFactory.Create(
        ...
        agentConfig: config,
        typeof(FileSystemPlugin),    // SubAgent can read files
        typeof(SearchPlugin)         // SubAgent can search
    );
}
```

The SubAgent only has access to the plugins you explicitly provide.
...
```csharp
public class DelegationMetadata : IPluginMetadata
{
...
| **Plugins** | Parent's | Parent's (referenced) | Own set |
...
3. **Provide only necessary plugins**: Don't give SubAgents access to plugins they don't need.
...
```csharp
// Good: Focused, well-instructed, minimal plugins
[SubAgent]
public SubAgent SecurityAuditor()
{
    ...
    return SubAgentFactory.Create(
        ...
        agentConfig: config,
        typeof(FileSystemPlugin)  // Only needs to read files
    );
}
...
public SubAgent DoAnything()
{
    return SubAgentFactory.Create(
        ...
        typeof(DatabasePlugin),
        typeof(NetworkPlugin),
        typeof(SystemPlugin)  // Way too many capabilities
    );
}
```
