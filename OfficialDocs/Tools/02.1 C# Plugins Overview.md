...
Plugins are the primary way to extend HPD-Agent with custom capabilities. A plugin is a **class** that contains one or more **capabilities** that the agent can use to accomplish tasks.
...
public class SearchPlugin
{
    ...
    public Skill ResearchSkill()
    {
        return SkillFactory.Create(
            ...
            instructions: "1. Search web first\n2. Search code if relevant\n3. Synthesize findings",
            "SearchPlugin.WebSearch", "SearchPlugin.CodeSearch"
        );
    }
}
```
...
A child agent that can be delegated tasks. SubAgents have their own configuration, plugins, and conversation thread.
...
Register plugins with the `AgentBuilder`:
...
If your plugin requires services, use `WithServiceProvider()`:
...
```csharp
var agent = new AgentBuilder()
    .WithPlugin<SearchPlugin>(new SearchMetadata
    {
        ProviderName = "Tavily",
...
When you have many plugins, the agent's context window can become cluttered. HPD-Agent supports **collapsing** plugins into containers that expand on demand.
...
Skills, SubAgents, and collapsed plugins all use **instructions** to guide the agent. Instructions are plain text strings with a few conventions:

```csharp
// Skill instructions - workflow guidance
SkillFactory.Create(
    name: "Research",
...
// Collapse instructions - FunctionResult (one-time) vs SystemPrompt (persistent)
[Collapse(
    description: "Database operations",
    FunctionResult: "Available: Query, Insert, Update, Delete",
