...
new FrontendPluginDefinition(
    ...
    StartCollapsed: true,
    FunctionResult: "IDE tools available: OpenFile, GetSelection, ShowMessage",
...
Provide guidance when plugins expand:

### FunctionResult (One-Time)

```csharp
new FrontendPluginDefinition(
    Name: "IDE",
    FunctionResult: @"
        IDE tools now available:
        - OpenFile: Open files in editor
        - GetSelection: Get current selection
        - ShowMessage: Display notifications

        Tip: Use GetSelection before making edits.",
    ...
)
```
...
Group tools into workflows with instructions:
