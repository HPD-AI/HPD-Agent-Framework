...
```csharp
public class MyMetadata : IPluginMetadata
{
...
### Feature-Gated Math Plugin

```csharp
public class MathContext : IPluginMetadata
{
...
```csharp
public class AppContext : IPluginMetadata
{
...
| Function always shown | No metadata passed | Ensure `.WithPlugin<T>(metadata)` is called |
...
- `{PluginName}Registry.g.cs` - Contains generated condition methods
