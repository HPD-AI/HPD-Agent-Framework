...
HPD-Agent uses a simple DSL (Domain Specific Language) for plugin instructions:
...
Skill instructions guide the agent through a multi-step workflow after the skill is activated.
...
SkillFactory.Create(
    ...
    instructions: @"
        ...
        5. Cite all sources used",                     // AFTER activation
    "SearchPlugin.WebSearch",
    "SearchPlugin.DeepRead"
);
```
...
instructions: "Use these functions to help the user"
```
...
## Collapse Instructions: FunctionResult vs SystemPrompt

The `[Collapse]` attribute has two instruction slots with different purposes:

```csharp
[Collapse(
    description: "Database operations",
    FunctionResult: "...",   // One-time, in conversation history
...
**Orientation and availability info:**
```csharp
FunctionResult: @"
    Database functions available:
    - Query: Read data (SELECT statements)
...
**One-time tips:**
```csharp
FunctionResult: "Tip: Use Query('SHOW TABLES') to see available tables before writing queries."
```
...
**Use FunctionResult for context:**
```csharp
// Good: Helpful orientation
FunctionResult: @"
    Expanded: ReadFile, WriteFile, DeleteFile, ListDir
...
[Collapse(description: "...", FunctionResult: "literal string")]
...
```csharp
// Bad: Won't work in skill instructions
instructions: "Search using {metadata.ProviderName}"

// Good: Use dynamic expression instead
instructions: nameof(GetInstructions)
...
|---------|-------------------|-----|
| FunctionResult | 3-10 lines | Orientation only, in history |
| SystemPrompt | 1-5 lines | Every turn, keep minimal |
| Skill Instructions | 5-20 lines | Workflow guidance |
...
Look for:
- `{PluginName}Registry.g.cs` - Plugin registration
...
|-----------------|-------|--------------|---------|
| Skill `instructions` | Function result | Once on activation | Workflow steps |
| `FunctionResult` | Function result | Once on expansion | Orientation, tips |
| `SystemPrompt` | System prompt | Every turn | Critical rules |
...
3. Put critical rules in SystemPrompt
4. Put orientation info in FunctionResult
5. Test your instructions—if the agent ignores them, they're too long or unclear
