...
**When the agent "calls" SearchTools:**
```
Agent: I'll use SearchTools to access search functions.
...
├── Agent calls FileTools container
...
#### Enabled
```csharp
Enabled = true  // Tools with [Collapse] are collapsible
...
public class GitTools { /* commit, push, pull, status, diff */ }

// Bad: Unrelated functions forced together
[Collapse("Misc utilities")]
public class MiscTools { /* sendEmail, readFile, getCurrentTime, translateText */ }
```
...
```
Tools: DatabaseTools (Query, Insert, Update, Delete, ...)
```
...
| Concept | Purpose |
|---------|---------|
| `[Collapse]` | Group tool functions into expandable container |
