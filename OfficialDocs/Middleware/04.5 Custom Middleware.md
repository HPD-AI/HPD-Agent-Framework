...
public class UserPreferencesMiddleware : IAgentMiddleware
{
    public Task BeforeMessageTurnAsync(BeforeMessageTurnContext context, CancellationToken ct)
    {
        ...
        var lang = context.Analyze(s =>
            s.MiddlewareState.UserPreferences?.Settings.GetValueOrDefault("language")
        );
    ...
    public Task AfterMessageTurnAsync(AfterMessageTurnContext context, CancellationToken ct)
    {
        ...
        if (DetectLanguagePreference(context.FinalResponse, out var newLang))
        {
            ...
            {
                var prefs = s.MiddlewareState.UserPreferences ?? new();
                return s with
                {
                    MiddlewareState = s.MiddlewareState.WithUserPreferences(prefs with
                    {
...
public class SummaryMiddleware : IAgentMiddleware
{
    ...
    {
        var summary = context.Analyze(s =>
            s.MiddlewareState.ConversationSummary?.Summary
        );
    ...
    {
        var messagesSummarized = context.Analyze(s =>
            s.MiddlewareState.ConversationSummary?.MessagesSummarized ?? 0
        );
        ...
        if (context.Messages.Count > messagesSummarized + 50)
        {
            ...
            context.UpdateState(s => s with
            {
                MiddlewareState = s.MiddlewareState.WithConversationSummary(new()
                {
                    Summary = newSummary,
...
See [05.2 Middleware State](./05.2%20Middleware%20State.md) for full persistence guide.
...
public class RateLimitMiddleware : IAgentMiddleware
{
    ...
    {
        var (windowStart, callsInWindow) = context.Analyze(s => (
            s.MiddlewareState.RateLimit?.WindowStart,
...
```csharp
//   GOOD: Null-coalescing with Analyze()
var count = context.Analyze(s => s.MiddlewareState.MyState?.Count ?? 0);

//   GOOD: Inside UpdateState lambda
context.UpdateState(s => {
