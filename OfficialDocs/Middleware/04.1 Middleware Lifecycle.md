...
public Task BeforeToolExecutionAsync(BeforeToolExecutionContext context, CancellationToken ct)
{
    ...
    foreach (var call in context.ToolCalls)
    {
        ...
        if (cbState.GetCount(call.Name, signature) >= 3)
        {
...
public Task AfterIterationAsync(AfterIterationContext context, CancellationToken ct)
{
    ...
    if (hasErrors)
    {
        ...
        {
            ConsecutiveFailures = errorState.ConsecutiveFailures + 1
        };
    }
    else
    {
        errorState = errorState with { ConsecutiveFailures = 0 };
    }
    ...
    CancellationToken ct)
{
    ...
    context.UpdateState(s => s with
    {
        MiddlewareState = s.MiddlewareState.WithBatchApprovals(approved)
    });
}
```
...
public Task BeforeFunctionAsync(BeforeFunctionContext context, CancellationToken ct)
{
    ...
    var approvals = context.State.MiddlewareState.BatchApprovals;
    if (approvals != null && !approvals.Contains(context.Function.Name))
    {
...
State is **always available** via `context.State`:

```csharp
var myState = context.State.MiddlewareState.MyCustomState ?? new();
var isTerminated = context.State.IsTerminated;
```
...
var newValue = context.State.MiddlewareState.MyCustomState;
```

**Updates are immediate** - no scheduled updates or pending state.

See [05.2 Middleware State](./05.2%20Middleware%20State.md) for details.
