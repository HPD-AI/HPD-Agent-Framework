...
public Task BeforeToolExecutionAsync(BeforeToolExecutionContext context, CancellationToken ct)
{
    ...
    foreach (var call in context.ToolCalls)
    {
        ...
        var count = context.Analyze(s =>
            s.MiddlewareState.CircuitBreaker?.GetCount(call.Name, signature) ?? 0
        );
...
public Task AfterIterationAsync(AfterIterationContext context, CancellationToken ct)
{
    ...
    context.UpdateState(s =>
    {
        ...
        var updated = hasErrors
            ? errorState with { ConsecutiveFailures = errorState.ConsecutiveFailures + 1 }
            : errorState with { ConsecutiveFailures = 0 };
...
Use `context.Analyze()` for safe state reads:

```csharp
var count = context.Analyze(s => s.MiddlewareState.MyCustomState?.Count ?? 0);
...
var newValue = context.Analyze(s => s.MiddlewareState.MyCustomState);
```
