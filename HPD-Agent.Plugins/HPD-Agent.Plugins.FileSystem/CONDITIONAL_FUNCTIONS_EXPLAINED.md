...
The `[ConditionalFunction]` attribute is one of the **coolest features** of your plugin architecture. It allows functions to **appear or disappear** from the AI's available tools based on runtime configuration!

## 🎯 The Problem It Solves

Imagine you have a FileSystem plugin, but you want different configurations for different scenarios:
...
1. Create multiple plugin classes, OR
...
{
    public bool EnableSearch { get; }  // ← The magic property!
...
public partial class FileSystemPlugin
{
    ...
    // ⚡ Only available when EnableSearch = true
    [AIFunction<FileSystemContext>]
    [ConditionalFunction("EnableSearch")]  // ← The conditional attribute!
...
## 🔧 Under the Hood: Source Generator Magic
...
private static bool EvaluateFindFilesCondition(IPluginMetadataContext? context)
{
    if (context == null) return true;

    if (context is not FileSystemContext typedContext)
        return false;

    try
    {
        // Evaluating: EnableSearch
        return typedContext.EnableSearch;  // ← Checks the property!
    }
    catch (Exception ex)
    {
...
public static List<AIFunction> CreateAllFunctions(IPluginMetadataContext? context)
{
    ...
    if (EvaluateFindFilesCondition(context))  // ← Runtime check!
    {
        functions.Add(CreateFindFilesFunction(...));
    }

    // Only add SearchContent if condition passes
    if (EvaluateSearchContentCondition(context))  // ← Runtime check!
    {
...
## 🌟 Real-World Example: Your WebSearch Plugin

Let's look at how you're already using this in your **WebSearchPlugin**:
...
Now you can have:
- **Production config**: `AllowDelete = false` → Delete function doesn't exist!
- **Development config**: `AllowDelete = true` → Delete function is available
...
└─────────────────┬───────────────────────────────────────────┘
                  ...
                  ▼
┌─────────────────────────────────────────────────────────────┐
│  2. Source Generator (Compile Time)                         │
│                                                             │
│  • Finds [ConditionalFunction] attribute                   │
│  • Validates "EnableSearch" exists in FileSystemContext    │
│  • Generates EvaluateFindFilesCondition() method           │
│  • Generates registration code with if check               │
└─────────────────┬───────────────────────────────────────────┘
                  │
                  ▼
┌─────────────────────────────────────────────────────────────┐
│  3. Runtime (User Creates Context)                          │
│                                                             │
│  var context = new FileSystemContext(enableSearch: false); │
└─────────────────┬───────────────────────────────────────────┘
                  │
...
│  (FindFiles and SearchContent are invisible!)              │
└─────────────────────────────────────────────────────────────┘
```
...
This makes your plugin system:
- **Flexible** - Same plugin, different configurations
