/**
 * Roving Focus Group utility for keyboard navigation
 *
 * Manages keyboard navigation between focusable elements in a group.
 * Supports arrow key navigation, Home/End, and looping.
 *
 * @example
 * ```typescript
 * const focusGroup = new RovingFocusGroup({
 *   rootNode: boxWith(() => containerRef),
 *   candidateAttr: 'data-tab-trigger',
 *   loop: boxWith(() => true),
 *   orientation: boxWith(() => 'horizontal')
 * });
 *
 * // In keydown handler:
 * focusGroup.handleKeydown(currentElement, event);
 * ```
 */

import { type Box, type ReadableBox, box } from 'svelte-toolbelt';
import { getElemDirection } from './locale.ts';
import { getDirectionalKeys } from './get-directional-keys.ts';
import { kbd } from './kbd.ts';
import type { Orientation } from './get-directional-keys.ts';
import { BROWSER } from 'esm-env';
import { isHTMLElement } from './is.ts';
	...
	getCandidateNodes() {
		...
		if (this.#opts.candidateSelector) {
			const candidates = Array.from(
				this.#opts.rootNode.current.querySelectorAll<HTMLElement>(this.#opts.candidateSelector)
			);
			return candidates;
