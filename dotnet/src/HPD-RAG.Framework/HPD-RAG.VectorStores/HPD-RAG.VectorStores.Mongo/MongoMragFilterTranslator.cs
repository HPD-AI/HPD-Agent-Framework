...
/// <summary>
/// Translates MragFilterNode AST to a MongoDB BsonDocument filter.
/// Uses standard MQL operators: $eq, $ne, $gt, $gte, $lt, $lte, $regex, $and, $or, $nor.
/// The "tag:{key}" property prefix maps to "tags.{key}" in the MongoDB document.
/// </summary>
internal sealed class MongoMragFilterTranslator : IMragFilterTranslator
{
    ...
    private static BsonDocument BuildBsonDocument(MragFilterNode node)
    {
        return node.Op switch
        {
            ...
            _ => throw new NotSupportedException($"MongoDB filter operator '{node.Op}' is not supported.")
        };
    }
