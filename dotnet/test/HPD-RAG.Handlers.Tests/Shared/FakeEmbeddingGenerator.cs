...
internal sealed class FakeEmbeddingGenerator : IEmbeddingGenerator<string, Embedding<float>>
{
    ...
    public FakeEmbeddingGenerator(int dimensions = 384) { _dimensions = dimensions; }
        ...
        EmbeddingGenerationOptions? options = null, CancellationToken cancellationToken = default)
    {
        var embeddings = values.Select(_ =>
            new Embedding<float>(Enumerable.Range(0, _dimensions)
                .Select(i => (float)i / _dimensions).ToArray())).ToList();
