...
/// <summary>
/// Unit tests for FrontendToolStateData immutable state management.
/// Tests all With* methods and state transitions.
/// </summary>
public class FrontendToolStateDataTests
{
    // ============================================
    // Constructor Tests
    // ============================================
    ...
    public void ClearPendingAugmentation_RemovesAugmentation()
    {
        // Arrange
        var state = new FrontendToolStateData()
            .WithPendingAugmentation(new FrontendToolAugmentation { ExpandPlugins = new HashSet<string> { "Plugin1" } });
    ...
    [Fact]
    public void MiddlewareState_FrontendTool_PropertyExists()
    {
        // Arrange
        var container = new MiddlewareState();

        // Act
        var state = container.FrontendTool;
    ...
    [Fact]
    public void MiddlewareState_WithFrontendTool_CreatesNewInstance()
    {
        // Arrange
        var container = new MiddlewareState();
        var testState = new FrontendToolStateData()
            .WithRegisteredPlugin(CreateTestPlugin("TestPlugin"));

        // Act
        var updated = container.WithFrontendTool(testState);

        // Assert
        Assert.NotSame(container, updated);
        Assert.NotNull(updated.FrontendTool);
        Assert.Single(updated.FrontendTool.RegisteredPlugins);
    }
    ...
    {
        // Arrange
        var original = new FrontendToolStateData();
