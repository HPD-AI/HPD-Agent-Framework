...
/// </list>
/// </remarks>
public sealed class LinuxSandboxEnhanced : IPlatformSandbox
{
    ...
    public async Task<string> WrapCommandAsync(string command, CancellationToken cancellationToken)
    {
        ...
        builder.WithDeniedWritePaths(_config.DenyWrite);
    ...
    private async Task EnsureInitializedAsync(CancellationToken cancellationToken)
    {
        ...
        if (needsNetwork && _httpProxy != null)
        {
            _socketBridge = new UnixSocketBridge(_logger);
            await _socketBridge.InitializeAsync(
                _httpProxy.Port,
                _socksProxy?.Port ?? _httpProxy.Port,
                cancellationToken);
