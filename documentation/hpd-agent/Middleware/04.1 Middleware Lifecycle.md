...
Execution flow:
```
RetryMiddleware.WrapModelCallStreamingAsync()
  → CachingMiddleware.WrapModelCallStreamingAsync()
    → LoggingMiddleware.WrapModelCallStreamingAsync()
      → Actual LLM call
```
...
- `RunConfig` - User's original options (read-only)
...
- `TurnUsage` - Token usage summed across all LLM iterations (`UsageDetails?`, null if provider returned no usage)
- `IterationUsage` - Per-iteration token usage breakdown (`ImmutableList<UsageDetails?>`)
- `RunConfig` - User's original options (read-only)
- `State` - Agent state
...
**When:** Wraps each LLM call. The only LLM wrap hook on `IAgentMiddleware`.
**Return `null`** (the default) to pass through without interception — this is the recommended no-op pattern.
...
**Use Cases:** Token counting, retry with buffering, mid-stream transformation, caching

**Example - Pass Through (no interception):**
```csharp
// Return null to opt out — the default implementation does this automatically
public IAsyncEnumerable<ChatResponseUpdate>? WrapModelCallStreamingAsync(
    ModelRequest request,
    Func<ModelRequest, IAsyncEnumerable<ChatResponseUpdate>> handler,
    [EnumeratorCancellation] CancellationToken ct) => null;
```
...
- `AllToolsSucceeded` - `true` if no tool threw an exception
- `AnyToolFailed` - `true` if any tool threw an exception
- `RunConfig` - Read-only user options
...
- `IsSuccess` - `true` if `Exception == null`
- `IsFailure` - `true` if `Exception != null`
...
See [04.3 Middleware Events](./04.3%20Middleware%20Events.md) for details.
...
| `Services` | `IServiceProvider?` | All hooks | No |
...
| `TurnUsage` | `UsageDetails?` | AfterMessageTurn | No |
| `IterationUsage` | `ImmutableList<UsageDetails?>` | AfterMessageTurn | No |
| `Iteration` | `int` | BeforeIteration+ | No |
| `IsFirstIteration` | `bool` | BeforeIteration | No |
...
| `AllToolsSucceeded` | `bool` | AfterIteration | No |
| `AnyToolFailed` | `bool` | AfterIteration | No |
...
| `IsSuccess` | `bool` | AfterFunction | No |
| `IsFailure` | `bool` | AfterFunction | No |
| `Exception` | `Exception?` | AfterFunction | No |
| `Error` | `Exception` | OnErrorAsync | No |
| `Source` | `ErrorSource` | OnErrorAsync | No |
| `Iteration` | `int` | OnErrorAsync | No |

## Next Steps

- [04.2 Middleware State](./04.2%20Middleware%20State.md) - Manage typed state
- [04.3 Middleware Events](./04.3%20Middleware%20Events.md) - Emit events and handle responses
- [04.4 Built-in Middleware](./04.4%20Built-in%20Middleware.md) - Ready-to-use middleware
- [04.5 Custom Middleware](./04.5%20Custom%20Middleware.md) - Build your own
