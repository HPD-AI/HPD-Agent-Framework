...
Use `[Collapse("description")]` to make a toolkit collapsible. This is the primary attribute for hierarchical collapsing:
...
**The description is shown as-is** when the container is presented to the agent. Write it to tell the agent what this group of tools does and when to open it.

**Note:** A toolkit without `[Collapse]` always has its functions visible directly.
...
#### Enabled
```csharp
Enabled = true  // Toolkits with [Collapse] are collapsible
...
Controls how many function names appear in the **auto-generated** container description for MCP servers (which don't have a hand-written `[Collapse]` description):
```csharp
MaxFunctionNamesInDescription = 10
// MCP container shows: "MCP Server 'filesystem'. Contains 3 functions: ReadFile, WriteFile, DeleteFile"

MaxFunctionNamesInDescription = 0
// MCP container shows: "MCP Server 'filesystem'."
```

This setting does not affect C# toolkits with `[Collapse]` — their description is always the string you provide.
...
**How it works:**
- The container is still generated at compile time (`[Collapse]` description still present)
...
```csharp
// Has [Collapse], so container is generated
[Collapse("File operations for reading, writing, and managing files")]
...
| Concept | Purpose |
|---------|---------|
| `[Collapse("description")]` | Group toolkit functions into expandable container |
| `FunctionResult` | One-time orientation message returned when the container is expanded |
