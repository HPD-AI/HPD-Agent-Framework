...
When marking state as `Persistent = true`, you can also control whether it is **branch-scoped** (default) or **session-scoped** using the `Scope` property:

```csharp
public enum StateScope
{
    Branch = 0,   // Default — each branch has its own copy; copied on fork
    Session = 1   // Shared across all branches; forks share the same data
}
```

```csharp
// Session-scoped: shared across all branches (e.g. permission choices, user preferences)
[MiddlewareState(Persistent = true, Scope = StateScope.Session)]
public sealed record PermissionPersistentStateData { }

// Branch-scoped (default): each conversation path has its own copy (e.g. plan progress)
[MiddlewareState(Persistent = true)]   // Scope = StateScope.Branch is the default
public sealed record PlanModePersistentStateData { }
```

**When to use `StateScope.Session`:**
- User permission choices ("Always Allow Bash") — should apply in every branch
- Environment settings that span all conversation paths

**When to use `StateScope.Branch` (default):**
- Plan progress, history caches — should diverge independently when you fork

On fork: session-scoped state is **shared** (one copy for all branches); branch-scoped state is **copied** (each fork starts with a snapshot that then diverges).
...
-   Agent restarts (if using a session store with persistence)
...
**How it works:**
- States marked `Persistent = true` are automatically saved to `Branch.MiddlewareState` (default, branch-scoped) or `Session.MiddlewareState` (if `Scope = StateScope.Session`)
- Loaded automatically when resuming from the same session/branch
...
var (session, branch) = await agent.LoadSessionAndBranchAsync("user-123");
await agent.RunAsync(["Set language to Spanish"], branch);
await agent.SaveSessionAndBranchAsync(session, branch);

// Run 2: Preferences are restored automatically
var (_, branch2) = await agent.LoadSessionAndBranchAsync("user-123");
await agent.RunAsync(["What's the weather?"], branch2);
...
- [04.3 Middleware Events](./04.3%20Middleware%20Events.md) - Emit events and wait for responses
- [04.4 Built-in Middleware](./04.4%20Built-in%20Middleware.md) - See state usage in real middleware
- [04.5 Custom Middleware](./04.5%20Custom%20Middleware.md) - Build your own with state
