...
public class JsonAgentPlanStore : AgentPlanStore
{
        ...
        try
        {
            ...
            foreach (var file in files)
            {
                try
                {
                    ...
                    var plan = JsonSerializer.Deserialize(json, MemoryJsonContext.Default.AgentPlan);
                    if (plan != null)
                    {
    ...
    private async Task SavePlanAsync(string conversationId, AgentPlan plan, CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            var json = JsonSerializer.Serialize(plan, MemoryJsonContext.Default.AgentPlan);
