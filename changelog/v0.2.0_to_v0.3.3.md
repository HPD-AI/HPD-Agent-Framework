...
This document catalogs all breaking changes in the HPD-Agent.Framework between versions 0.2.0 and 0.3.3. The period contains significant architectural refactoring including session/thread separation, memory system overhaul, graph feature expansion, and new observability infrastructure.
...
2. [Session & Thread Management](#session--thread-management)
...
### Monolithic AgentSession → Separated Session + Thread Model

**Breaking Change:** Fundamental architecture shift from single-message-store to multi-thread design.
...
public class Session
{
    ...
    // Messages now in Thread objects
}
...
1. **Session Loading:** Must now load both Session metadata AND Thread (with messages):
   ```csharp
   // Before
...
2. **Session Persistence:** Methods now work with metadata-only Session and separate Thread:
   ```csharp
   // Before
   ...
   await store.SaveThreadAsync(sessionId, thread);  // Messages and history
   ```
...
3. **ISessionStore Interface:** Completely redesigned:
   ...
   - Added: `LoadThreadAsync`, `SaveThreadAsync`, `ListThreadIdsAsync`, `DeleteThreadAsync`
...
**New State Scoping:**
```csharp
// Separate scopes: Session-scoped vs Thread-scoped
public class StateScope
{
    public Dictionary<string, object> SessionScoped { get; }    // Shared across threads
    public Dictionary<string, object> ThreadScoped { get; }     // Per-thread
}
...
[MiddlewareState(scope: StateScope.Thread)]
public class MyHistoryMiddleware { }
```

**Attribute Changes:**
- New: `[MiddlewareState(scope)]` attribute for declaring state ownership
- Modified: `MiddlewareStateContainer` now manages both session and thread scopes
- Impact: Middleware storing per-thread data must explicitly declare thread scope
...
### Priority 1: Session/Thread Architecture
...
1. **Update Session Loading Code:**
   ...
   // After
   var session = await store.LoadSessionAsync(sessionId);
   var threadId = session.Metadata.TryGetValue("defaultThread", out var bid)
       ? bid.ToString()
       : "main";
   var thread = await store.LoadThreadAsync(sessionId, threadId);
   var messages = thread.Messages;
...
2. **Update Session Persistence:**
   ...
   var thread = new Thread { Id = threadId, SessionId = sessionId, Messages = ... };
   await store.SaveThreadAsync(sessionId, thread);
   ```
...
- **Custom ISessionStore:** Must implement new thread-focused methods:
  ```csharp
  public Task<Thread?> LoadThreadAsync(string sessionId, string threadId, ...) { }
  public Task SaveThreadAsync(string sessionId, Thread thread, ...) { }
  public Task<List<string>> ListThreadIdsAsync(string sessionId, ...) { }
  public Task DeleteThreadAsync(string sessionId, string threadId, ...) { }
  ```
  Remove checkpoint-related methods.

- **Custom Middleware:** Declare state scope:
  ```csharp
  [MiddlewareState(scope: StateScope.Session)]  // or StateScope.Thread
...
|----------|------|--------|--------|
| Session Architecture | Breaking | AgentSession → Session + Thread | Must Update |
...
**Key Commits:**
- `e4f256f` - SubAgents: thread-aware thread modes
