...
public class UnifiedScopingManager
{
    ...
    private ContainerType GetContainerType(AIFunction function)
    {
        ...
        // Check for IsScope flag (highest priority - [Scope] attribute)
    ...
    private enum ContainerType
    {
        ...
        /// <summary>
        /// Container created by [Scope] attribute on skill class WITH skills.
        /// Example: [Scope("Analysis")] on FinancialAnalysisSkills class that has [Skill] methods
        ...
        /// Example: [Scope("Math")] on MathPlugin class with only [AIFunction] methods
    ...
    private bool IsScopeContainerVisible(AIFunction container, VisibilityContext context)
    {
        ...
        {
            _logger?.LogDebug($"[SCOPING] Scope container {scopeName}: HIDDEN (implicitly registered via skills)");
            return false;
        }
        ...
        {
            _logger?.LogDebug($"[SCOPING] Scope container {scopeName}: HIDDEN (expanded)");
            return false;
        }

        _logger?.LogDebug($"[SCOPING] Scope container {scopeName}: VISIBLE (not expanded)");
        return true;
    }
    ...
    private bool IsSkillContainerVisible(AIFunction container, VisibilityContext context)
    {
        ...
        {
            _logger?.LogDebug($"[SCOPING] Skill container {skillName}: HIDDEN (skill expanded)");
            return false;
        }
        ...
        {
            _logger?.LogDebug($"[SCOPING] Skill container {skillName}: VISIBLE (no parent scope)");
            return true;
        }
        ...
        {
            _logger?.LogDebug($"[SCOPING] Skill container {skillName}: VISIBLE (parent scope {parentScope} doesn't exist)");
            return true;
        }
        ...
        {
            _logger?.LogDebug($"[SCOPING] Skill container {skillName}: VISIBLE (parent scope {parentScope} expanded)");
            return true;
        }

        // Otherwise: parent scope not expanded - hide
        _logger?.LogDebug($"[SCOPING] Skill container {skillName}: HIDDEN (parent scope {parentScope} not expanded)");
        return false;
    }
    ...
    private bool IsSkillVisible(AIFunction skill, VisibilityContext context)
    {
        ...
        {
            _logger?.LogDebug($"[SCOPING] Skill {skill.Name}: VISIBLE (no parent container)");
            return true;
        }

        if (context.ExpandedSkillContainers.Contains(parentContainer))
        {
            _logger?.LogDebug($"[SCOPING] Skill {skill.Name}: VISIBLE (parent container {parentContainer} expanded)");
            return true;
        }

        _logger?.LogDebug($"[SCOPING] Skill {skill.Name}: HIDDEN (parent container {parentContainer} not expanded)");
        return false;
    }
    ...
    ///   - If plugin has [Scope] container AND function is referenced by an expanded skill → VISIBLE (skill bypass)
    ///   - If plugin has [Scope] container AND parent plugin is expanded → VISIBLE
    ///   - If plugin has [Scope] container AND not expanded → HIDDEN
    ...
    private FunctionVisibility GetFunctionVisibility(AIFunction function, VisibilityContext context)
    {
        ...
        // PRIORITY 1: If plugin has [Scope] container, check skill bypass first
        if (parentPlugin != null && context.PluginsWithContainers.Contains(parentPlugin))
        {
            ...
            if (context.FunctionsReferencedBySkills.Contains(functionName))
            {
                ...
                if (anySkillExpanded)
                {
                    _logger?.LogDebug($"[SCOPING] Function {functionName}: VISIBLE (skill bypass for scoped plugin {parentPlugin})");
            ...
            {
                _logger?.LogDebug($"[SCOPING] Function {functionName}: VISIBLE (scoped parent {parentPlugin} expanded)");
                return FunctionVisibility.ExpandedPlugin;
            }

            _logger?.LogDebug($"[SCOPING] Function {functionName}: HIDDEN (scoped parent {parentPlugin} not expanded)");
        ...
        if (parentPlugin != null && _explicitlyRegisteredPlugins.Contains(parentPlugin))
        {
            ...
            _logger?.LogDebug($"[SCOPING] Function {functionName}: VISIBLE (explicitly registered plugin)");
        ...
        if (context.FunctionsReferencedBySkills.Contains(functionName))
        {
            ...
            if (anySkillExpanded)
            {
                _logger?.LogDebug($"[SCOPING] Function {functionName}: VISIBLE (referenced by expanded skill)");
                return FunctionVisibility.ExpandedSkill;
            }

            // If no skills expanded, function is hidden
            _logger?.LogDebug($"[SCOPING] Function {functionName}: HIDDEN (referenced by skills, none expanded)");
            return FunctionVisibility.Hidden;
        }
            ...
            functionName.Equals("ReadSkillDocument", StringComparison.OrdinalIgnoreCase))
        {
            ...
            {
                _logger?.LogDebug($"[SCOPING] Function {functionName}: VISIBLE (skill with documents expanded)");
                return FunctionVisibility.ExpandedSkill;
            }

            // If no skill with documents is expanded, function is hidden
            _logger?.LogDebug($"[SCOPING] Function {functionName}: HIDDEN (no skill with documents expanded)");
            return FunctionVisibility.Hidden;
        }
        ...
        if (parentPlugin != null && context.PluginsWithScopedSkills.Contains(parentPlugin))
        {
            ...
            _logger?.LogDebug($"[SCOPING] Function {functionName}: HIDDEN (orphan in implicitly-registered plugin {parentPlugin})");
            return FunctionVisibility.Hidden;
        }

        // DEFAULT: Non-scoped function - always visible
        _logger?.LogDebug($"[SCOPING] Function {functionName}: VISIBLE (non-scoped, default)");
        return FunctionVisibility.NonScoped;
    }
