...
/// <summary>
/// Input configuration for agent execution with frontend tool support.
...
{
    /// <summary>
    /// Frontend plugin containers to register for this message turn.
    /// Each plugin contains tools that can be collapsed/expanded together.
    /// This is the ONLY way to register frontend tools - matching HPD's plugin-centric model.
    /// </summary>
    public IReadOnlyList<FrontendPluginDefinition>? FrontendPlugins { get; init; }
    ...
    /// Unlike State (which is frontend app state), Metadata is for
    /// integration-level concerns (tracing, routing, feature flags, etc).
    /// </summary>
    public object? Metadata { get; init; }

    /// <summary>
    /// If true, clears all frontend tool state from previous turns.
    ...
    /// </list>
    /// </remarks>
    public bool ResetFrontendState { get; init; } = false;
}
